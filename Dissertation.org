...
*Tais also made some experiments that concludes the same thing.*
...
data from the trace file and puts it in memory. 
...
Then, the EventDecoder breaks it into events identifying, line by
line, the event's fields and creating an object with all the necessary
