...
the JVM. The *guarani* platform was uncapable of handling A and B sizes.
Our analysis is unaffected by these missing cases since it is
possible to observe a pattern which is the opposite of what we have
expected.
We can see that the smaller the batch size, the better the
