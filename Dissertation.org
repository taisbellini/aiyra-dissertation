...
trace file always goes through all the packages. First, the file is
...
simulation based on the event type. This simulation always generates
an entity, even if incomplete (with no end timestamp, for
example). Last, this new entity is sent to the plugin, which contains
specific entry points for every different kind of entity.
...
to ignore pending links, for example. As in the =PajeNG=, all the
...
are either in the PajeSimulator or in the PajeContainer. This
structure 
...
Whenever a type of any kind is defined, it is sent to a unique point
=newType=. There is a method in the type objects that indicates what
is the nature of that object (Container, State, Event, Variable or
Link). When a container is created, it is forwarded to the
...
timestamp as -1. The link states have three instrumentation points:
=startLink=, =endLink= and =newCompletedLink=. At anytime a variable
is set or updated, there are three entities sent to the plugin: the
first, which contains the first value of the variable; the last, which
is the one immediately before the variable in question; and the new
variable. The aditions and subtractions are sent to the same point
(=updateVar=).
...
of the simulation itself, whithout the interference of the other
segments.

** Paje Dump Plugin

The Dump plugin just outputs the entities generated by the
simulator. The implementation is simply to add a print function in
each instrumentation point that receives a complete entity. The
difference between the =PajeDumpPlugin= and the =pj_dump= tool is that
the first one dumps the information as soon as the entity is
completed, whithout keeping it in memory. With this approach, it is
possible to solve the issue regarding the need to wait for the program
to end to have the results.

This plugin can be called with the argument =pjdump= in the =-p=
option and adds a new argument (=-l=) which can group together a
certain number of entities before dumping it. The option receives an
integer as parameter. This option gives a little more flexibility  
