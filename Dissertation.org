...
a trace file always goes through all of the packages. First, the trace
...
the inclusion of new ones. A grammar (*PajeGrammar.jj*) describing the
elements of the Pajé file format, presented in section
\ref{section.pajeformat}, was created to be processed by the Java
Compiler Compiler. The implementation uses the JavaCC syntax. Each
event definition is stored in an array, while the events are simulated
...
\ref{ap.optionshandler} and \ref{ap.grammar}, respectively.
...
implementation was the extending the output of the simulator. This
makes it more limited for extensions in the Pajé Trace file, but it is
simple to adjust it and the changes would not affect the
...
password (=-pwd=). The default for these
options is: *localhost*, *root* and *root*, respectively.
...
To execute aiyra navigat to the *workspace* folder and execute the
...
The database for the mysql plugin is in the script pajeDB.sql.
