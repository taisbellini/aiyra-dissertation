...
chance in this case. Likewise, the *pj_dump* implementation carries
the overhead of storing everything. Another important difference among
Aiyra and PajeNG is the process of reading the trace file, parsing it
and then sending to the simulator. As seen in Chapter
\ref{chapter.paje}, the PajeNG tool first reads from the file a chunk
of data, then the a decoder that breaks it into events and then the
simulator receives this block *(?)*. The next segment of data is only
read after the first is completely decoded. On the other hand, Aiyra's
controller sends each event read immediately to the simulator. Since
the memory allocation is not usually a problem in C++ implementations,
it is more likely that this result relies on the architectural
...
changes architectural approach may have significan impacts in the
