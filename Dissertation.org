...
the Java language is that it generates source code in Java, which
...
better performance than other tools such as ANTLR (Another Tool For
Language Recognition), that requires a runtime library
\cite{javaccversusantlr}. ANTLR was our first choice of parser
generator, but it was soon discarded due to its low performance.
...
As an example, We will use =MyGrammar.jj=. The result of the
processing are seven files: =MyGrammar.java=, the parser;
=MyGrammarTokenManager.java=, the lexical analyser, that manages the
tokens; =MyGrammarConstants.java=, some useful constants. The other
four files generated: =Token.java=, =TokenMgrError.java=,
...
grammar itself.  Once the corresponding Java source code for the
scanner and parser are generated, compile them as usual with =javac=.
...
There is a specific terminology used in experimental design: =Response
Variable=, the outcome of an experiment; =Factors=, all of the variables
that could have several different values affecting the response
variable; =Levels=, the possible values that a factor can assume;
=Primary Factors=, the factors that need to be quantified; =Secondary
Factors=, the factors whose impacts in the performance are not
relevant for the analysis; =Replication=, the repetition of all or
some experiments; =Design=, the specification of total number of
experiments, based on factor level combination and number of
replications for each experiment; =Experimental Unit=, the entity used
for the experiment, for example, a computer; =Interaction=, when the
levels of a fator affect the results of other factor.
...
design, it is possible to evaluate factors that have different numbers
of levels. The advantage of this model is that every possible
...
the =DoE.base= package.  This package contains the class _design_ with
several accessor functions to create different types of design. One
particular important function is the _fac.design_, which creates full
factorial designs with arbitrary numbers of levels. The function
receives several arguments, including number of factors, levels and
...
=blocks=: Number of blocks in which the experiment will be
...
execution of a parallel program and replays them in simulation. It is
developed to simulate trace files in the Pajé Trace File Format, thus,
it is important to understand how the Pajé trace files are
composed. Section \ref{section.pajeformat} describes this format and
all entities types. The next section describes the PajeNG
implementation focusing on the =libpaje= module, which is where the
core simulation is performed. Since the new generation follows the
exact same structure as the original Pajé, it will be the only one to
be described.
...
generic format that describes the behavior of parallel and distributed
programs. This format contains three sections: event definition, type
...
even the containers themselves. Below, a succinct definition of each
type of entity.
...
of a variable along time. A variable is represented by an object with
a value and two timestamps, beginning and end, indicating how long the
variable had that specific value.

*Links*: A link represents a relationship between two containers, such
as a communication between processes. It contains two timestamps
...
The format is divided in two sections: event definition and events.

*Event Definition* 

The first part of a trace file describes all the possible events of
...
which was defined previously, followed by the fields separated by
space or tab. Before the entities, such as states or links, can be
...
container. There are sets of events associated to each kind of entity
...
auxiliar tools to manage the trace files. The base for the
...
#+CAPTION: Image taken from \cite{kergommeaux2000paje}
[[file:~/TCC/aiyra-dissertation/img/paje-parco.png]]
...
every type and entity, to the most specific. There are three main
hierarchies that are particularly important in this objective: one for
events, one for types and one for entities. With this modular
implementation, it is relatively easy to add a new type of event or
entity and integrate it with the rest of the code.
...
[[file:~/TCC/aiyra-dissertation/img/eventsHierarchy.png]]
...
[[file:~/TCC/aiyra-dissertation/img/typesHierarchy.png]]
...
[[file:~/TCC/aiyra-dissertation/img/entitiesHierarchy.png]]
...
generated is added to the =typeMap=. 
...
three packages: parser, simulator core and plugin. Every event of a
trace file always goes through all of the packages. First, the file is
read by the parser, where a trace event object is created. This
instance contains the type of event in question and the field
values. Then, the simulator receives this object and executes the
simulation based on the event type. The simulation always generates an
entity, even if incomplete (with no end timestamp, for example). Last,
this new entity is sent to the plugin, which contains specific entry
points for every different kind of entity.

#+CAPTION: Aiyra's Architecture
#+NAME: Aiyra
[[file:~/TCC/aiyra-dissertation/img/aiyraArchitecture.png]]
...
presented in chapter \ref{chapter.plugins}.
...
=PajeNG= implementation, having the same class hierarchies. It
implements only the essential functionalities, leaving aside the
possibility of defining a specific timestamp to end the simulation or
to ignore pending links, for example. As in the =PajeNG=, all of the
...
Every trace event has an instrumentation point, which dispatches the
entity objects generated to the plugin. These points are either in the
PajeSimulator or in the PajeContainer. In the =PajeSimulator= are all
of the outputs regarding the definition of types, the creation of
containers and the definition of values. Although the containers are
not complete objects, since they don't have ending timestamp or the
related entities, they are sent anyway with the alias and type
information. The =PajeContainer= in in charge of dispatching to the
plugin the instances related to it, which involve the states, events,
links and variables. It also may send unfinished objects.
...
approach. A full list of the plugin entraces and the information
received in each one is presented in the next section.

** Plugin

The plugin package is composed by an abstract class with one method
for each instrumentation point presented above. It also contains a
method called =finish()= where the user can perform some final actions
...
differences are too little and not worth of a full plugin
implementation.

The entrances of the plugins comprise the definition of types and
values, the creation of containers and the formation of new
entities. Whenever a type of any kind is defined, it is sent to a
unique point =newType=, with the =PajeType= object as argument. The
new values are received in the =newValue= entry point. When a
container is created, the instance is forwarded to the
=newCreatedContainer= entry point, with the end timestamp set to
-1. Whenever an entity is not completed yet, it will have the end
timestamp as -1. The destroyed containers are sent to the
=destroyedContainer= method. The link states have three
instrumentation points: =startLink=, a communication link entity with
unknown end time and container; =endLink=, when the instance has the
end point of the link but not the start; and =newCompletedLink=, when
there is a link entity with beginning and end. Anytime a variable is
set or updated, there are three entities sent to the plugin: the
...
=newVar= variable. The aditions and subtractions are sent to the same
point (=updateVar=). The set, push and pop state events each have one
method: =setState=, =pushState=, =popState=. The events are sent to
...
In this chapted, two plugins are described: =PajeDumpPlugin= and
...
in each instrumentation point that receives a complete entity. When it
is a destroyed container, it is necessary to iterate over the entities
left in the container. The difference between the =PajeDumpPlugin= and
the =pj_dump= tool is that the first one outputs the information as
soon as the entity is completed, whithout keeping it in memory. With
...
before dumping the entities. This option gives a little more
flexibility for the user and may improve the performance, since the
printing function of Jave costs time. For it to be possible, a
=StringBuilder= is used as a buffer keeping all of the output until it
reaches the number of lines informed.
...
next subsections. Thsi plugin allows the user to save data from
multiple files in the same database.

The plugin can be used by specifying =mysql= as argument for the =-p=
option. It is necessary to have a MySQL connection with a database
with the correct schema. To specify the server of the connection,
there is the option =-s=. It is also possible to inform a username
(=-u=), a password (=-pwd=) and a database name (=-d=). The default
for these options is: =localhost=, =root=, =root=, =paje=,
respectively.
...
The entity-relationship (ER) model contains one entity for each type
of Pajé object. Also, to support multiple files, there is a =file=
entity, which has the =name=, a =comment= and the =date= as
attributes, as well as a =file_id=. The =type= and =container=
entities have an identifying relationship with =file=, which means
that the file id is part of their primary key. The relationship is a
one to many, since a file can have multiple types and containers.

The =type= entity has =alias=, composing the primary key, =name= and
...
multiple children. Link and Variable types have exclusive attributes
that are not common to all types, thus, both are specific entities
that ihnerit from =type=. =LinkType= adds a relationship with itself
to represent the communication with start and end. This is a many to
many relationship because the types can be start and end to various
communications. The =VariableType= has a color attribute.

The =container= entity has and identifying relashionship with =type=
and contains the same attributes as type, including the =parent= one
to many relation. The =value= entity is also a weak entity related to
=type=, and has =alias= (primary key), =name= and =color= as
attributes. Every e in the Pajé format is identified by its
