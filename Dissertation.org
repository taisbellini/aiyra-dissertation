...
_Comment_:
- Integrate this last paragraph in previous ones
...
(DBMS) in a standard manner.

_Comment_:
- Missing year in the citation
...
as a native C library to allow developers to SQL commands  directly in
...
_Comments_:
- Jain citation is completely wrong. Reference the second edition of
...
experiments, which can cost a lot of
...
(the result of $3*2*20$).

When there are too many factors and levels, it may not be
...
the execution of a program and replays them through simulation.
...
generation (PajeNG) follows the exact same structure as the original
Pajé, it will be the only one to be detailed.
...
container C2, of type T2, the type T2 must be child of T1 in the type
...
generation of the Pajé simulator (PajeNG).
...
_Comments_:
- Again, you should present the Figure before in the paragraph.

#+BEGIN_LaTeX
\begin{figure}[!htb]
\caption{TODO: Say what the figure is then put the "inspired from" between brackets. Figure inspired in \cite{kergommeaux2000paje}}
\centering
...
_Comment about this figure_:
- It is wrong. What is the cylinder?
...
is a class containing all the fields read by the =PajeEventDecoder=. As
depicted in Figure \ref{fig.eventsHierarchy}, the event
...
_Comments regarding par. below_:
- is "origin" a verb?
...
\ref{fig.entitiesHierarchy} represents this structure.
...
this novel approach, developed in our work, will be detailed in the next chapter.
...
also has a string that defines the =key= and start and end containers
...
types and values and the creation of containers. Although in this
point the containers are not complete objects, since they don't have
ending timestamp or the related entities, they are forwarded anyway
with the alias and type information. The =PajeContainer= is in charge
of dispatching to the plugin the instances related to it, which
involve the states, events, links and variables. It also may send
unfinished objects. When there is a =PajeDestroyContainerEvent=, the
container object is sent again, now complete with an ending timestamp.
...
values, the creation of containers, the formation and completion of
new entities. The *newType* entry point is a unique entrance for when
a type of any kind is defined, having the =PajeType= object as
...
not completed yet, the end timestamp will be set to =-1=. The
...
simulation, but the destroyed container may have some remaining one
that coundn't be excluded, such as variables.
...
*newCompletedLink*, where the link entity has beginning and
...
iterate over the entities left in the container. The variables are
also printed in the destruction of the container, since they are not
...
The *Type* entity has =alias=, composing the identifier, =name= and =depth= as
attributes. It also contains a self-referencing one to many relationship to
indicate the =parent= type, as a type can have multiple children. It is
associated to a *Value* entity, which describes the =PajeValue= class, with
=alias=, =name=, =type= and =color=. Link and variable types have exclusive
attributes that are not common to all types, thus, both are specializations of
*Type*. *LinkType* adds a relationship with itself to represent a the start
and end. This is a many-to-many relationship because the types can be the start
and end to various communications. The =VariableType= has a color attribute.
...
including the =parent= one-to-many relation. The *Value* entity has
=alias= (identifier), =name= and =color= as attributes. 

All of the entities that are related to a container, have an
identifying one-to-many relation with *Container* entity. The relation
of *Container* with *State* has =start= and =end= attributes, where
=start= is identifier. *Link* has two relationships with *Container*, one for
=startContainer= and one for =endContainer=. Since it has a unique
=key= attribute, the containers are not identifiers. A *Variable*
entity contains the =startTime= attribute identifiying it, as well as
an =updateTime= in the relation with *Container*. Also, this relation
has a =value= attribute. The *Event* entity has a =time= field.
...
After the creation of the conceptual model, a translation to a logical model
was made. In this conversion, besides applying the universally known rules
presented in chapter \ref{chapter.basic_concepts}, we considered the usability
of the schema, analysing the common consultations made in Pajé data. This
reflection is a usual part of the process, where the needs of the client are
contemplated.

The entities defined in the ER Model all became tables. For the *Type*
specialization, we used the first option presented in
\ref{subsection.er_model}: combining everything in a single table with the
following fields: =file_id=, =alias=, =name=, =depth=, =parent_type_alias=,
=start_link_type=, =end_link_type= and =color=. =file_id=, inherited from the
identifying relation with *File*, and =alias= compose the primary key. The
self-referencing relationships are described as foreign keys in their tables.
The entities associated to the container all have at least three foreign keys
that are also identifiers: =type_alias=, =container_alias= and =file_id=.
*Link* has two foreign keys from *Container* but they don't belong to the
identifier.

In our ER Model, the value is only associated to the *Type*, thus, if one
wants to know the value of a state, for example, it needs to first get its
type, then, go to *Type* table to retrieve the value. Since it is desirable to
easily get an entity's value, we added a relationship between the valued
entities (*State*, *Link* and *Variable*) with *Value*. =value_alias= is an
identifying foreign key for all, except *Link*, where the identifier consists
only in the =key=, =type= and =file_id=.
