...
that are not common to all types, thus, both are specializations of
*Type*. *LinkType* adds a relationship with itself to represent a the
start and end. This is a many-to-many relationship because the types
can be start and end to various communications. The =VariableType= has
a color attribute.
...
=start= is identifier. *State* is also identified by its one-to-one
relationship with *Value*, as well as the other valued
entities. *Link* has two relationships with *Container*, one for
