...
extending the Pajé format, we identified three main issues in the
...
simulation. The plugin that inserts the data in a MySQL database is the
only one that brings an extremely different functionality to the
...
To build a grammar to be compiled by =JavaCC= you only need to create
one file whith `.jj` extension. The structure of this file is the
following:
