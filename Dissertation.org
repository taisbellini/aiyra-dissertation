...
=PajeSimulator= or in the =PajeContainer=. In the =PajeSimulator= are
all of the outputs regarding the definition of types, the creation of
...
is a =PajeDestroyContainerEvent=, the container is sent again, now
complete with an ending timestamp.
...
representing its fields. A =PajeType= contains =alias=, =name=,
=depth=, and =parent=, which is a =PajeType= as well. it also provides
the =getNature()= method, to identify which entity this type
describes. The nature is an enumeration and can assume
...
=startType= and =endType=, which represent the type of the containers
of start and end of the communication. A value is represented by a
=PajeValue= class, with =name=, =alias=, =type= and =color= as
attributes.

A container object (=PajeContainer=) contains an =alias=, =name=,
=type= and =parent= (another =PajeContainer=), besides the structures
to store the entities related to it, as described in section
\ref{section.pajeng}. As the container class is a child of the
=PajeDoubleTimedEntity=, it also has =startTime= and =endTime=. 
