...
(We performed an experimental evaluation comparing Aiyra to the
existing approach. The separation of the core simulation from the data
handling provided very good results. We were able to perform a
simulation two times faster than the original tool. This is very
relevant, since it opens several possibilities of creating high
performance solutions. Thus, besides building an extensible simulator,
we also proved it to have better performance than the existing tool.)
NAO PRECISA MAIS. So mencionar esse experimento e dar enfase na
relevancia.
