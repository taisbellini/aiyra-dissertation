...
entity belongs to a container.

There are sets of events associated to type definitions and the manipulation
of each kind of entity. The events have some specific characteristics and must
follow some rules about its fields. For example, an entity with container C1
as parent can only be instantiated after the creation of container C1.
...
more common to have all the types defined in the beginning. 

The events associated to the containers are timestamped and can create or
destroy instances during the trace file. A container cannot be referenced after
it was destroyed. 

An event is something that is relevant enough to be acknowledged and
the events associated to this occasions have a unique timestamp.

Variables can be set at a specific timestamp and have its value changed
throughout the simulation by addition and subtraction events. The value of a
variable is a double precision floating-point number, which is different from
the values of the other entities. A variable must be set before changes to its
value can be made.

A link can start at a container and end in another one. There is a field with
a unique key that identifies the connection.
...
structure that list all of the objects related to the container (=entities=),
there are auxiliar structures for some specific type, such as states
(=stackStates=) and links (=pendingLinks=). There is some repetition between
