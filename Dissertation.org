...
distribution of resource usage, while the local environment is
overloaded by the MySQL server. However, this would have affected the
other inputs as well, which was not the case. We do not have a
conclusion about this result and it is open for further analysis.
...
The objective of this work was to build a simulator that would give
...
must be a careful analysis on the configuration of the JVM to assure
...
explored. There can be evaluations testing different configurations
